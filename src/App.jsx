...
import ProjectOverview from "./components/Pages/ProjectOverview.jsx";
  ...
  const App = () => {
    ...
    const [entered, setEntered] = useState(false); // Tracks whether the user has clicked 'Enter'
  
    return (
      <ModeProvider>
        <LoaderProvider sources={sources}>
          <div className="app">
            {/* Canvas3D appears only if entered is true */}
            <Canvas3D
              entered={entered}
            ...
            {/* Show LoadingScreen only if entered is false */}
            {!entered && <LoadingScreen setEntered={setEntered} />}
  
            {/* Show Router content after user has entered */}
            {entered && (
              <Router>
            ...
            {/* Popup stays visible regardless of entered */}
